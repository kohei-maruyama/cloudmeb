...
from cloudmeb.receipt_bank_txts.models import Receipt_bank_txt
...
def email(request):
    status = 0
    data = request.POST
    if all (key in data for key in ('message', 'first_name', 'last_name' 'email')):
        website = Website.objects.get(website_id=settings.SITE_ID)
        ...
        message = '<p><b>PageName:</b>&nbsp;Contact Us</p>' + '<p><b>Name:</b>&nbsp;' + data.get('first_name') + ' ' + data.get('last_name') + '</p>' + '<p><b>Email:</b> &nbsp;' + data.get('email') + '</p>' + '<p><b>Message:</b> &nbsp;' + data.get('message') + '</p>'
        contactmail = EmailMessage(subject, message, '%s <%s>' % ('Cloudmeb server', website.smtp_from_email), ['%s <%s>' % ('Cloudmeb admin', website.smtp_from_email)], [], connection=connection, reply_to=['%s %s <%s>' % (data.get('first_name') + ' ' +data.get('last_name'), '', data.get('email'))])
        contactmail.content_subtype = "html"
        status = contactmail.send()
    return HttpResponse(status)
...
def emailsec(request):
    status = 0
    data = request.POST
    if all (key in data for key in ('company', 'first_name', 'last_name', 'email', 'pagename' , 'pageurl')):
        website = Website.objects.get(website_id=settings.SITE_ID)
        connection = get_connection(backend='django.core.mail.backends.smtp.EmailBackend', host=re.sub('^https?://', '', website.smtp_server), port=website.smtp_port, username=website.smtp_user, password=decode_AES(website.smtp_password), use_tls=website.smtp_tls)
        subject = 'Cloudmeb Mail'
        message = '<p><b>PageName:</b>&nbsp;' + data.get('pagename') + '</p>' + '<p><b>Name:</b>&nbsp;' + data.get('first_name') + ' '+ data.get('last_name') + '</p>' + '<p><b>Company:</b> &nbsp;' + data.get('company') + '</p>' + '<p><b>Email:</b> &nbsp;' + data.get('email') + '</p>' + '<p><b>PhoneNumber:</b> &nbsp;' + data.get('phone') + '</p>' + '<p><b>PageURL:</b>&nbsp;' + data.get('pageurl') + '</p>'
        partnermail = EmailMessage(subject, message, '%s <%s>' % ('Cloudmeb server', website.smtp_from_email), ['%s <%s>' % ('Cloudmeb admin', website.smtp_from_email)], [], connection=connection, reply_to=['%s %s <%s>' % (data.get('first_name') + ' ' + data.get('last_name'), '', data.get('email'))])
        partnermail.content_subtype = "html"
        status = partnermail.send()
...
@ensure_csrf_cookie
def receipt_bank_landing(request):
    try:
        if (get_language()[:2] == 'en'):
            model = Product.objects.get(solution_slug_en="receipt-bank-cloud-receipt-scanning-software")
        else:
            model = Product.objects.get(solution_slug_fr="receipt-bank-nuage-recus-scanner-logiciel")

        return render_to_response('receipt_bank_landing.html', {
            'base_url': get_base_url(request),
            'page': Page.objects.get(page_name='RECEIPT_BANK_LANDING'),
            'product': model,
            'website': Website.objects.get(website_id=settings.SITE_ID),
            'shared_txts': Shared_txt.objects.all(),
            'receipt_bank_txt': Receipt_bank_txt.objects.get(receipt_bank_txt_name='RECEIPT_BANK_LANDING'),
            'packages_tables': Packages_table.objects.all(),
            'services': Service.objects.order_by('service_order').all()
        }, context_instance = RequestContext(request))
...
@ensure_csrf_cookie
def receipt_bank_landing_freetrial(request):
    try:
        if (get_language()[:2] == 'en'):
            model = Product.objects.get(solution_slug_en="receipt-bank-cloud-receipt-scanning-software")
        else:
        ...
        return render_to_response('receipt_bank_landing_freetrial.html', {
            'base_url': get_base_url(request),
            'page': Page.objects.get(page_name='RECEIPT_BANK_LANDING_FREETRIAL'),
            'product': model,
            'website': Website.objects.get(website_id=settings.SITE_ID),
            'shared_txts': Shared_txt.objects.all(),
            'receipt_bank_txt': Receipt_bank_txt.objects.get(receipt_bank_txt_name='RECEIPT_BANK_LANDING'),
